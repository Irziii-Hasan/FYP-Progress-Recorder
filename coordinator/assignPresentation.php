<?php
include 'config.php';
include 'session_coordinator2.php'; // Add session
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $externalEvaluators[] = $row;
    }
}

// Fetch event types from the events table
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $room_id = htmlspecialchars($_POST["room_number"]);
    $date = htmlspecialchars($_POST["date"]);
    $time = htmlspecialchars($_POST["time"]);
    $project_id = htmlspecialchars($_POST["projects"]);
    $internalEvaluators = $_POST["internalEvaluators"];
    $externalEvaluator_id = isset($_POST["externalEvaluator"]) ? htmlspecialchars($_POST["externalEvaluator"]) : null; // Optional external evaluator

    // Check for past dates
    $currentDate = date('Y-m-d');
    if ($date < $currentDate) {
        echo '<script>alert("You cannot assign a presentation to a past date!"); window.history.back();</script>';
        exit;
    }

    // Check if the room is available at the selected date and time
    $stmt = $conn->prepare("SELECT * FROM presentations WHERE room_id = ? AND date = ? AND time = ?");
    $stmt->bind_param("iss", $room_id, $date, $time);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        echo '<script>alert("The selected room is already reserved at this time. Please choose another room or time."); window.history.back();</script>';
        exit;
    }

        // Retrieve external evaluator ID, default to NULL if not selected
        $externalEvaluator_id = isset($_POST["externalEvaluators"]) && !empty($_POST["externalEvaluators"]) 
        ? htmlspecialchars($_POST["externalEvaluators"][0]) 
        : null; // Handle external evaluator as an array


    // Insert presentation data
    foreach ($internalEvaluators as $internalEvaluator_id) {
        $stmt = $conn->prepare("INSERT INTO presentations (type, batch, room_id, date, time, project_id, internal_evaluator_id, external_evaluator_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die('Prepare failed: ' . htmlspecialchars($conn->error));
        }

        // Bind parameters, external_evaluator_id will be NULL if not provided
        $stmt->bind_param("iiissiii", $type, $batch, $room_id, $date, $time, $project_id, $internalEvaluator_id, $externalEvaluator_id);

        if ($stmt->execute()) {
            echo '<script>alert("Presentation successfully scheduled!"); window.location.href = "view_schedule.php";</script>';
        } else {
...
// Fetch available rooms based on selected time and date
if (isset($_GET['date']) && isset($_GET['time'])) {
    $date = $_GET['date'];
    $time = $_GET['time'];

    $stmt = $conn->prepare("SELECT room_id, room_number FROM rooms WHERE room_id NOT IN (SELECT room_id FROM presentations WHERE date = ? AND time = ?)");
...
// Fetch projects based on batch and exclude those already scheduled for the selected event
if (isset($_GET['batch']) && isset($_GET['type'])) {
    $batch_id = $_GET['batch'];
    $type_id = $_GET['type'];
    $projects = [];
    ...
    if ($batch_id && $type_id) {
        $stmt = $conn->prepare("
            ...
            FROM projects 
            WHERE batch = ? 
            AND id NOT IN (SELECT project_id FROM presentations WHERE type = ?)
        ");
        $stmt->bind_param("ii", $batch_id, $type_id);
        $stmt->execute();
        $result = $stmt->get_result();
