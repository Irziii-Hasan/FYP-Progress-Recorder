...
$sql = "SELECT * FROM meetings WHERE id = ? AND supervisor_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $meeting_id, $_SESSION['faculty_id']);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    die("Meeting not found or you don't have permission to edit this meeting.");
}
...
$timeValue = $meeting['time'];
$projectValue = $meeting['project_id'];

// Fetch projects assigned to supervisor
$projects = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($titleError) && empty($dateError) && empty($timeError) && empty($projectError) && empty($descriptionError)) {
        ...
        $sql = "UPDATE meetings SET title = ?, description = ?, date = ?, time = ?, project_id = ? WHERE id = ? AND supervisor_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssiii", $title, $description, $date, $time, $project, $meeting_id, $_SESSION['faculty_id']);
      ...
      <div class="col-md-12">
        ...
        <div class="container mt-3 form-all" style="width: 650px;">
          <h2 class="text-center form-heading">Schedule Meeting</h2>
            ...
            <input type="date" class="form-control" id="date" name="date" value="<?php echo htmlspecialchars($dateValue); ?>" required>
