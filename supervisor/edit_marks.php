...
// Get the internal evaluator ID from the session
$internal_evaluator_id = $_SESSION['faculty_id']; // Adjust the session variable name as per your session management
    ...
    if ($valid) {
        ...
        foreach ($marks as $index => $mark) {
            ...
            $sql_update = "UPDATE marks SET marks = ? 
                           WHERE student_id = ? AND form_id = ? AND project_id = ? AND internal = ? AND description_id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("iiiiii", $mark, $student_id, $form_id, $project_id, $internal_evaluator_id, $description_id);
            $stmt_update->execute();
        ...
        $stmt_total_update->bind_param("iiiii", $total_marks, $student_id, $form_id, $project_id, $internal_evaluator_id);
...
$sql = "SELECT m.description_id, m.marks, fd.description, fd.max_marks 
        ...
        WHERE m.student_id = ? AND m.form_id = ? AND m.project_id = ? AND m.internal = ?";

$stmt = $conn->prepare($sql);
$stmt->bind_param("iiii", $student_id, $form_id, $project_id, $internal_evaluator_id);
$stmt->execute();
