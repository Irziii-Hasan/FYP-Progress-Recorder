...
$external_evaluator_id = $_SESSION['external_id']; // Adjust the session variable name as per your session management
...
$username = $result_user->fetch_assoc()['username'] ?? 'Unknown User'; // Default to 'Unknown User' if not found
    ...
    if ($valid) {
        ...
        foreach ($marks as $index => $mark) {
            ...
            $sql_update = "UPDATE marks SET marks = ? 
                           WHERE student_id = ? AND form_id = ? AND project_id = ? AND external = ? AND description_id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("iiiiii", $mark, $student_id, $form_id, $project_id, $external_evaluator_id, $description_id);
            $stmt_update->execute();
        ...
        $sql_total_update = "UPDATE external_total_student_marks SET total_marks = ? 
                             WHERE student_id = ? AND form_id = ? AND project_id = ? AND external_id = ?";
        $stmt_total_update = $conn->prepare($sql_total_update);
        $stmt_total_update->bind_param("iiiii", $total_marks, $student_id, $form_id, $project_id, $external_evaluator_id);
        $stmt_total_update->execute();

        // Redirect back to the same page after updating to avoid form resubmission
        header("Location: edit_marks.php?student_id=$student_id&form_id=$form_id&project_id=$project_id&presentation_id=$presentation_id&success=1");
        exit();
    }
}
...
$sql = "SELECT m.description_id, m.marks, fd.description, fd.max_marks 
        ...
        WHERE m.student_id = ? AND m.form_id = ? AND m.project_id = ? AND external = ?";

$stmt = $conn->prepare($sql);
$stmt->bind_param("iiii", $student_id, $form_id, $project_id, $external_evaluator_id);
$stmt->execute();
...
<head>
    ...
    <style>
        ...
        .marks-summary {
            margin-top: 20px; /* Add some space above marks summary */
        }
        .alert {
            margin-top: 20px; /* Add some space above alert */
        }
    </style>
</head>
<body>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <h3 class="heading mb-4">Edit Marks for Student: <?php echo htmlspecialchars($username); ?></h3>
                    ...
                    <form action="" method="post">
                        <div class="table-container">
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        <th>Description</th>
                                        <th>Marks</th>
                                        <th>Max Marks</th> <!-- New column for Max Marks -->
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if ($result->num_rows > 0): ?>
                                        <?php while ($row = $result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" value="<?php echo htmlspecialchars($row['max_marks']); ?>" class="form-control" readonly> <!-- Display Max Marks -->
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else: ?>
                            ...
                            <!-- Displaying Passing Marks and Total Marks within the form -->
                ...
                <?php if (isset($_GET['success'])): ?>
                    <div class="alert alert-success">Marks have been updated successfully!</div>
                <?php endif; ?>
