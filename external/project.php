<?php
include 'session_external_evaluator.php'; // Ensure this is after session_start()

// Check if external_id is set
if (!isset($_SESSION['external_id']) || !is_numeric($_SESSION['external_id'])) {
    die('External ID is not set or is invalid.');
}
...
try {
    // Prepare SQL statement to fetch projects for the external supervisor
    $sql = "SELECT * FROM projects WHERE external_supervisor = ?";
    ...
    // Fetch names for students, co-supervisors, supervisors, and external supervisors
    $names = [];
    $student_ids = [];
    $faculty_ids = [];
    $external_ids = [];
    foreach ($projects as $project) {
        ...
        $external_ids[] = $project['external_supervisor'];
    }
    ...
    if (!empty($external_ids)) {
        ...
        $external_ids_list = implode(',', array_filter($external_ids));
        if (!empty($external_ids_list)) {
            $sql = "SELECT external_id, name FROM external WHERE external_id IN ($external_ids_list)";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $names[$row['external_id']] = $row['name'];
                }
            }
        }
    }
    ...
    <div class="container-fluid" id="content">
        <div class="row">
            <div class="col-md-12">
                ...
                <!-- Pages Heading -->
                <h2 style="text-align: center;" class="heading">View Projects</h2>
                ...
                <div class="tab-content" id="projectTabsContent">
                    <?php foreach ($projects as $index => $project): ?>
                        <div class="tab-pane fade <?php echo $index === 0 ? 'show active' : ''; ?>" id="project-<?php echo $project['id']; ?>" role="tabpanel" aria-labelledby="tab-<?php echo $project['id']; ?>">
                            ...
                            <p><strong>External Supervisor:</strong> <?php echo isset($names[$project['external_supervisor']]) ? htmlspecialchars($names[$project['external_supervisor']]) : 'N/A'; ?></p>
                            <p><strong>Created At:</strong> <?php echo date('Y-m-d', strtotime($project['created_at'])); ?></p>
                        </div>
                    <?php endforeach; ?>
                </div>
